  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
      ...
      <div ref={scrollRef} className="overflow-x-auto select-none" style={{ height: BAR_MAX_H + 34 }}>
        <div className="flex items-end" style={{ width: days.length * COL_W + 16, padding: "6px 8px 0", height: BAR_MAX_H + 34, position: "relative" }}>
          ...
          {days.map((day) => {
            ...
            return (
              <div key={day.date} ref={isToday ? todayRef : undefined}
                className="flex flex-col items-center shrink-0 group" style={{ width: COL_W }}
                title={`${day.date} · ${total.toLocaleString()} / ${cap.toLocaleString()} đôi`}>
                <div className="relative w-full rounded-t-sm overflow-hidden" style={{
                  height: BAR_MAX_H,
                  ...
                  {segs.map((seg, si) => (
                    <div key={seg.order_id + si} className="absolute left-0 right-0" style={{
                      bottom: `${seg.yPct * 100}%`, height: `${seg.p * 100}%`,
                      background: seg.color, opacity: isPast ? 0.4 : 0.82, minHeight: seg.p > 0 ? 1 : 0,
                    }} />
                  ))}
                  {fillPct >= 0.95 && <div className="absolute top-0 left-0 right-0 h-1 bg-red-400 opacity-60" />}
                  {isToday && <div className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-px bg-red-400" />}
                </div>
                <div className={clsx("text-center font-medium", isToday ? "text-red-600 text-[9px] font-bold" : "text-[9px] text-gray-400")}
                  style={{ lineHeight: "14px", paddingTop: 2 }}>
                  {isToday ? "HN" : day.date.slice(8)}
                </div>
                {day.date.slice(8) === "01" && (
                  <div className="text-[8px] text-gray-300" style={{ lineHeight: "10px" }}>T{parseInt(day.date.slice(5, 7))}</div>
                )}
              </div>
            );
          })}
